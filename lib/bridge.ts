...
export async function generateBlueprint(prompt:string,provider:AIProvider={}):Promise<BuildAction[]> {
  ...
  const limitInstruction="The effective blueprint limit is 64 actions, overriding any earlier 36-action wording when the requested quality needs more components. ";
  ...
  if(!Array.isArray(actions)||actions.length>64) throw new Error("AI provider returned an invalid blueprint");
  ...
  for(let attempt=0;attempt<1&&weakness;attempt++){
    try{
      ...
      const repaired=parseActions(raw);if(Array.isArray(repaired)&&repaired.length<=64&&blueprintScore(repaired,prompt)>=blueprintScore(actions,prompt))actions=repaired;
    }catch{}
