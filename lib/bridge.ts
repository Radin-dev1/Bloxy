...
function normalize(actions:BuildAction[]){
  ...
  for(let pass=0;pass<4;pass++)actions.forEach(action=>{if(action.type==="create_instance"&&(action.parent==="StarterGui"||uiNames.has(action.parent)||/^(ScreenGui|Frame|ScrollingFrame|ImageLabel|ImageButton|TextLabel|TextButton|TextBox|UI)/.test(action.className||"")))uiNames.add(action.name)});
  return actions.map((action,index)=>{
    ...
    const geometric=properties.Position!==undefined||properties.Size!==undefined,isUI=uiNames.has(action.name);
    ...
    if(isUI){
      ...
      else if(properties.Text!==undefined||/title|label|text|desc|count|price|rarity/i.test(action.name))inferredUI="TextLabel";
      else inferredUI="Frame";
    }
    const className=action.className||inferredUI||(namedClasses.has(action.name)?action.name:geometric?"Part":action.name.endsWith("Model")?"Model":undefined);
...
async function askAI(system:string,prompt:string,provider:AIProvider={}){
  ...
  if(id==="bloxy"||id==="gemini"){
    ...
    const request={method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":key},body:JSON.stringify({system_instruction:{parts:[{text:system}]},contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{temperature:.15,maxOutputTokens:16384,responseMimeType:"application/json"}})};
    let response=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`,request);
    if(!response.ok&&id==="bloxy"&&model!=="gemini-3.1-flash-lite")response=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite:generateContent",request);
...
export async function generateBlueprint(prompt:string,provider:AIProvider={}):Promise<BuildAction[]> {
  ...
  const limitInstruction="The effective blueprint limit is 128 actions, overriding any earlier lower limit when the requested quality needs more components. Prefer 24-72 purposeful actions and never add filler. ";
  let raw=await askAI(system+assetInstruction+uiInstruction+limitInstruction,prompt,provider);
  ...
  if(actions.length>128)throw new Error(`AI provider returned ${actions.length} actions, above the safe limit of 128`);
  let weakness=weakDraftReason(actions,prompt);
  for(let attempt=0;attempt<1&&weakness;attempt++){
    try{
      raw=await askAI(system+assetInstruction+uiInstruction+limitInstruction,`Rebuild this weak draft. Problem: ${weakness} Preserve the request but return a complete replacement JSON array. User request: ${JSON.stringify(prompt)} Weak draft: ${JSON.stringify(actions)}`,provider);
      const repaired=parseActions(raw);if(Array.isArray(repaired)&&repaired.length<=128&&blueprintScore(repaired,prompt)>=blueprintScore(actions,prompt))actions=repaired;
    }catch{}
