...
type CreatorStoreAsset = {
  ...
  asset?: {
    ...
    name?: string;
    description?: string;
    creator?: CreatorStoreAsset["creator"];
  };
};
  ...
  return assetList(payload)
    .map((item) => {
      const asset = item.asset || item,
        id = Number(asset.id || item.id),
        creator = asset.creator || item.creator || {};
      return {
        id,
...
export async function GET(request: Request) {
  ...
  let assets;
  try {
    assets = await searchCreatorModels(query, 18);
  } catch (error) {
    return json(
      {
        error:
  ...
  if (assets.length) {
    ...
    const thumbPayload = (await fetch(thumbnails)
      .then((result) => result.json())
      .catch(() => ({ data: [] }))) as { data?: Array<{ targetId?: number; imageUrl?: string }> };
    const imageById = new Map(
      (thumbPayload.data || []).map((image) => [Number(image.targetId), image.imageUrl || ""]),
    );
    assets.forEach((asset) => {
      asset.thumbnail = imageById.get(asset.id) || "";
    });
  }
