    ...
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-image:generateContent",
    {
      ...
      body: JSON.stringify({
        contents: [
          {
            role: "user",
            parts: [{ inlineData: { mimeType: match[1], data: match[2] } }, { text: instruction }],
          },
        ],
...
async function tripo(path: string, init: RequestInit = {}) {
  ...
  if (!response.ok || payload.code !== 0)
    throw new Error(payload.message || `Tripo request failed (${response.status})`);
  return payload.data || {};
}

export async function POST(request: Request) {
  const auth = bearer(request);
  if (!auth) return json({ error: "Pair Roblox Studio first" }, 401);
  const body = (await request.json()) as GenerateBody,
    prompt = (body.prompt || "").trim().slice(0, 1024);
  ...
  const referenceId = crypto.randomUUID(),
    charge = await debit(auth, "3d_textured", referenceId);
  if (!charge.ok)
    return json(
  ...
  try {
    ...
    if (body.imageData) {
      const concept = await conceptFromDoodle(
        body.imageData,
        prompt,
        body.style || "Roblox low-poly",
      );
      const form = new FormData();
      form.append("file", new Blob([concept.bytes], { type: concept.mime }), "bloxy-concept.png");
      const uploaded = await tripo("/upload/sts", { method: "POST", body: form });
      const token = String(uploaded.image_token || uploaded.file_token || "");
      if (!token) throw new Error("Tripo upload returned no image token");
      task = await tripo("/task", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          face_limit: body.assetPack ? 8000 : 5000,
          texture: true,
          pbr: true,
        }),
      });
      ...
      task = await tripo("/task", {
        ...
        body: JSON.stringify({
          type: "text_to_model",
          model_version: "P1-20260311",
          prompt: `${prompt}. ${body.style || "Roblox low-poly"}, game-ready asset`,
          face_limit: body.assetPack ? 8000 : 5000,
          texture: true,
        }),
      });
    ...
    await refund(charge.walletId, "3d_textured", referenceId);
    return json({ error: error instanceof Error ? error.message : "3D generation failed" }, 502);
  }
}
