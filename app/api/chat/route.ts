...
export async function POST(request:Request){
  ...
  const body=await request.json() as {prompt?:string;provider?:{id?:string;model?:string;apiKey?:string}};
  const prompt=(body.prompt||"").trim().slice(0,3000);
  if(!auth||!prompt)return json({error:"Missing session or prompt"},400);
  const database=await db();
  const session=await database.prepare("SELECT id,status,request_count,expires_at FROM bridge_sessions WHERE web_token_hash=? AND status IN ('waiting','connected')").bind(await digest(auth)).first<{id:string;status:string;request_count:number;expires_at:number}>();
  ...
  const jobId=crypto.randomUUID(),charge=await debit(auth,"blueprint",jobId);
  if(!charge.ok)return json({error:charge.error,balanceBits:charge.balance,requiredBits:charge.required},402);
  try{
    const provider=body.provider?{id:String(body.provider.id||"").slice(0,30),model:String(body.provider.model||"").slice(0,180),apiKey:String(body.provider.apiKey||"").slice(0,500)}:undefined;
    const actions=await generateBlueprint(prompt,provider);
